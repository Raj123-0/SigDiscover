...
def plot_signature_profile(signature: np.ndarray, title: str = "Mutational Signature", save_path: Optional[str] = None) -> plt.Figure:
    fig, ax = plt.subplots(figsize=(15, 4))
    colors, mutation_types, bases = ['#03BDFF', '#000000', '#E41A1C', '#A6CEE3', '#1F78B4', '#FB9A99'], ['C>A', 'C>G', 'C>T', 'T>A', 'T>C', 'T>G'], ['A', 'C', 'G', 'T']
    x = np.arange(96)
    for i, color in enumerate(colors): ax.bar(x[i*16:(i+1)*16], signature[i*16:(i+1)*16], color=color, width=0.8)
    ax.set_xticks(x)
