...
def plot_cosine_heatmap(discovered: np.ndarray, cosmic: np.ndarray, cosmic_names: List[str], save_path: Optional[str] = None) -> plt.Figure:
    ...
    ax.set_title("Cosine Similarity: De Novo vs COSMIC"); plt.xticks(rotation=90); plt.tight_layout()
    if save_path:
...
def plot_rank_selection(rank_results: pd.DataFrame, save_path: Optional[str] = None) -> plt.Figure:
    ...
    ax1.tick_params(axis='y', labelcolor='tab:blue'); ax1.axhline(y=0.8, color='gray', linestyle='--', alpha=0.7); ax1.set_ylim(0, 1.05)
    ax2 = ax1.twinx()
    ax2.set_ylabel('Reconstruction Error', color='tab:red'); ax2.plot(k_vals, recon_err, marker='s', color='tab:red', linewidth=2); ax2.tick_params(axis='y', labelcolor='tab:red')
    ...
    fig.suptitle("Rank Selection Criteria vs. Number of Signatures (k)"); ax1.set_xticks(k_vals); fig.legend(loc='upper right', bbox_to_anchor=(0.9, 0.9)); plt.tight_layout()
    if save_path:
        ensure_dir(os.path.dirname(save_path)); fig.savefig(save_path, dpi=300, bbox_inches='tight'); plt.close(fig)
    return fig